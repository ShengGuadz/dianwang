...
# 通过二分查找法找到最接近目标比特率（target bpp）的JPEG图像质量参数
...
# 将图像文件转换为二进制位字符串，并将其保存到指定的输出文件中
def img_to_bit(input_path, output_path):
...
# 将输入字符串按指定长度切割成多个子字符串，并返回这些子字符串组成的列表
...
# 从指定路径的文本文件中读取比特流，并将其转换为由0和1组成的数组
def optimized_get_bitarray(txt_path):
    ...
    return bit_array
# 从指定路径读取一个包含二进制字符串的文本文件，并将其转换为一个比特流数组
...
if __name__ == "__main__":
    ...
    # 输入图片数据集根父路径
    input_base_path = './data/media/Dataset/origin_clic2024_numbered'

    # 输入的图片格式(后缀名，区分大小写)
    input_fmt = 'png'

    # 压缩后的JPEG图片数据集根父路径（未进行LDPC+QAM+AWGN）
    output_base_path = './data/media/Dataset/clic_origin_jpeg'

    # 图片对应字节比特流txt的目录
    output_txt_path = './data/media/Dataset/clic_jpeg_txt'

    # 指定SNR
    snr = 11

    # 输出通过了信道传输的JPEG图片数据集根父路径（经过了LDPC+QAM+AWGN）
    channelcoded_output_base_path = '/home/ubuntu/users/zhuxiangben/WITT/data/media/Dataset/test_jpeg_out/'+str(snr)+'dB'
